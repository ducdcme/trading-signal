...
import { fetchOkxPayload } from "./okx.js";
...
export async function resolveActiveSpotMarket(value, priority = ["BINANCE", "OKX", "BYBIT"], quotePriority = ["USDT", "USDC", "FDUSD"]) {
  const requested = typeof value === "string" ? parseInstrument(value) : value;
  if (!requested?.asset) throw new MarketNotFoundError("Mã coin không hợp lệ");
  const compact = String(typeof value === "string" ? value : requested.instrumentId || requested.asset).toUpperCase().replace(/[^A-Z0-9]/g, "");
  const explicitQuote = ["FDUSD", "USDT", "USDC"].find(quote => compact.endsWith(quote));
  const quotes = explicitQuote ? [explicitQuote] : quotePriority;
  const apiErrors = [];

  for (const exchange of priority) {
    let catalog;
    try { catalog = await fetchActiveSpotCatalog(exchange); }
    catch (error) { apiErrors.push(`${exchange}: ${error.message}`); continue; }
    const exchangeQuotes = exchange === "BINANCE" ? quotes : quotes.filter(quote => quote !== "FDUSD");
    for (const quote of exchangeQuotes) {
      const candidate = parseInstrument(`${exchange}:${requested.asset}${quote}`);
      const active = catalog.get(candidate.instrumentId);
      if (active) return active;
    }
  }

  if (apiErrors.length === priority.length) throw new Error("Không thể kiểm tra thị trường Spot lúc này; xem chi tiết trong log server");
  throw new MarketNotFoundError(`Không tìm thấy ${requested.asset} trên ${priority.join(" → ")}`);
}

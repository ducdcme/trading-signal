...
export async function fetchGateClosedCandles(symbol, timeframe = "1D", limit = 500, includeOpen = false) {
  ...
  }).filter(item => includeOpen || item.closeTime < now).sort((a, b) => a.openTime - b.openTime), "Gate.io", symbol);
}

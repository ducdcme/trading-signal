...
export function dueAutomationJobs(clock, settings, config) {
  ...
  return [
    ...
    { key: "metalsDaily", timeframe: "1D", assetGroup: "metals", due: schedules.metalsDaily?.enabled && clock.time === schedules.metalsDaily.time },
    { key: "stockDaily", timeframe: "1D", assetGroup: "stocks", due: settings.assets?.stocks?.enabled && schedules.stockDaily?.enabled && clock.day >= 1 && clock.day <= 5 && clock.time === schedules.stockDaily.time },

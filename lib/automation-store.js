...
export const AUTOMATION_SCHEMA_VERSION = 11;
...
export const DEFAULT_AUTOMATION = Object.freeze({
  ...
  assets: {
    ...
    stocks: { enabled: false, watchlist: [], scopes: ["WATCHLIST"], provider: "SSI" }
  },
  schedules: {
    ...
    stockDaily: { enabled: false, time: "07:00" },
...
export function normalizeAutomation(input = {}) {
  ...
  const stockDailyInput = input.schedules?.stockDaily ?? { enabled: false };
  const migratedStockDaily = sourceSchemaVersion < 11 && stockDailyInput?.time === "15:30"
    ? { ...stockDailyInput, time: "07:00" }
    : stockDailyInput;
  ...
  return {
    ...
    assets: {
      ...
      stocks: {
        ...
        scopes: uniqueStrings(stocks?.scopes ?? ["WATCHLIST"], 5).map(value => value.toUpperCase()).filter(value => ["WATCHLIST", "VN30", "HOSE", "HNX", "UPCOM"].includes(value)),
        provider: "SSI"
      }
    },
    schedules: {
      ...
      stockDaily: normalizeDaily(migratedStockDaily, DEFAULT_AUTOMATION.schedules.stockDaily),
...
export async function loadAutomationState(path) {
  return readJson(path, { schemaVersion: AUTOMATION_SCHEMA_VERSION, sentKeys: [], lastRuns: {}, lastSlots: {}, batchSlots: {} });
}
...
export async function saveAutomationState(path, state) {
  const safe = {
    ...
    batchSlots: Object.fromEntries(Object.entries(state.batchSlots && typeof state.batchSlots === "object" ? state.batchSlots : {}).slice(-200))
  };

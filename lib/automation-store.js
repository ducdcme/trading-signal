...
export const DEFAULT_AUTOMATION = Object.freeze({
  ...
  telegram: { chatId: "", sendNoSignalSummary: true, sendErrors: true },
  assets: {
...
function normalizeFocusSchedule(input) {
  const minute = Number(input?.minute);
  return { enabled: input?.enabled !== false, minute: Number.isInteger(minute) && minute >= 0 && minute <= 59 ? minute : 5 };
}
...
export function normalizeAutomation(input = {}) {
  ...
  return {
    ...
    telegram: {
      ...
      sendErrors: (telegram?.sendErrors ?? input.sendErrors) !== false
    },
    assets: {
      ...
      dex: {
        ...
        watchlist: (Array.isArray(dex?.watchlist ?? input.dexTokens) ? (dex?.watchlist ?? input.dexTokens) : [])
          .map(item => ({ network: String(item.network ?? "").trim().toLowerCase(), tokenAddress: String(item.tokenAddress ?? "").trim() }))
    ...
    schedules: {
      ...
      focusScan: normalizeFocusSchedule(input.schedules?.focusScan),

export function signalKey(row, timeframe) {
  return [row.assetType || "CEX", row.exchange, row.instrumentId || row.symbol, row.network || "", row.tokenAddress || "", timeframe, row.candleOpenTime, row.status, ...(row.buyTypes || []), ...(row.sellTypes || [])].join("|");
}

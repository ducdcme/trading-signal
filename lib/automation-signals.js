export function signalKey(row, timeframe) {
  ...
  return [row.assetType || "CEX", exchange, instrumentId, row.network || "", row.tokenAddress || "", row.poolAddress || "", timeframe, row.candleOpenTime, row.status, ...(row.buySignalTypes || row.buyTypes || []), ...(row.sellSignalTypes || row.sellTypes || [])].join("|");
}

export function signalKey(row, timeframe) {
  ...
  const marketIdentity = [row.assetType || "CEX", exchange, instrumentId, row.network || "", row.tokenAddress || "", row.poolAddress || ""];
  if (row.side) marketIdentity.push(row.side);
  return [...marketIdentity, timeframe, row.candleOpenTime, row.status, ...(row.buySignalTypes || row.buyTypes || []), ...(row.sellSignalTypes || row.sellTypes || [])].join("|");
}

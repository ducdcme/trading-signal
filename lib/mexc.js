...
export async function fetchMexcClosedCandles(symbol, timeframe = "1D", limit = 500) {
  ...
  const now = Date.now();
  return ensureCandles(payload.filter(row => Number(row[6]) < now).map(row => ({ openTime: Number(row[0]), open: Number(row[1]), high: Number(row[2]), low: Number(row[3]), close: Number(row[4]), volume: Number(row[5]), closeTime: Number(row[6]) })), "MEXC", symbol);
}

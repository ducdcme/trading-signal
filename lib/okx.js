...
export async function fetchOkxPayload(url) {
...
export async function fetchOkxClosedCandles(instrumentId, timeframe = "1D", limit = 500, includeOpen = false) {
  ...
  while (byTimestamp.size < limit) {
    ...
    for (const row of rows) {
      if (!includeOpen && row[8] !== "1") continue;
...
export const fetchOkxClosedDailyCandles = (symbol, limit = 500, includeOpen = false) => fetchOkxClosedCandles(symbol, "1D", limit, includeOpen);

...
export function isNewCoinScheduleDue(clock, schedule, settings, minuteAfterClose = 5) {
  const config = normalizeNewCoinConfig(settings);
  const [hour, minute] = String(clock?.time || "").split(":").map(Number);
  const scheduledMinute = Number(minuteAfterClose);
  return schedule?.enabled !== false
    && config.scanHours.includes(hour)
    && minute === (Number.isInteger(scheduledMinute) && scheduledMinute >= 0 && scheduledMinute <= 59 ? scheduledMinute : 5);
}
